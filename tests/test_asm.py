...
from pz80.asm import Asm, assemble
from pz80.z80 import Z80


@pytest.fixture
def assembler():
    """Asmクラスのインスタンスを提供するフィクスチャ"""
    return Asm()
...
def test_instructions_with_only_reserved_words(assembler, instruction, expected_opcode):
    """予約語のみで構成される命令のアセンブルをテストする"""
    src = [{"line": 1, "asm": instruction}]
    asm_list = assembler.pass0(src)
...
def test_db_instruction_string(assembler):
    """DB命令（文字列）のアセンブルテスト"""
...
def test_db_instruction_mixed(assembler):
...
def test_dw_instruction_numeric(assembler):
    """DW命令（数値）のアセンブルテスト"""
...
def test_dw_instruction_label(assembler):
    """DW命令（ラベル）のアセンブルテスト"""
    # ORG 0x1000 を設定
...
def test_dw_instruction_multiple_labels(assembler):
...
def test_undefined_label_in_instruction(assembler):
    """未定義ラベル（通常命令）のエラーテスト"""
...
def test_undefined_label_in_dw(assembler):
    """未定義ラベル（DW命令）のエラーテスト"""
    # dw UNDEFINED_LABEL
...
def test_db_instruction_string_with_escape(assembler):
...
def test_char_literal_instruction(assembler):
...
def test_char_literal_instruction_word(assembler):
...
def test_dw_instruction_string_literal(assembler):
...
def test_dw_instruction_long_string_literal_error(assembler):
    ...
    with pytest.raises(ValueError, match="String literal in DW must be 1 or 2 characters"):
        assembler.pass1(asm_list)
def test_db_instruction_out_of_range_error(assembler):
    """DB命令（範囲外の数値）のエラーテスト"""
...
def test_bit_instruction(assembler):
    """BIT命令のテスト"""
    # bit 7, a は CB 7F になる
...
def test_res_instruction(assembler):
    """RES命令のテスト"""
...
def test_set_instruction(assembler):
    """SET命令のテスト"""
    # set 1, c は CB C9 になる
...
def test_bit_ix_instruction(assembler):
    """BIT命令(IX)のテスト"""
...
def test_set_iy_instruction_with_register(assembler):
...
def test_jr_instruction_backward(assembler):
...
def test_equ_directive(assembler):
    """EQU疑似命令のテスト"""
...
def test_label_with_offset(assembler):
...
def test_expression_with_precedence(assembler):
    """乗算と加算を含む数式評価のテスト"""
    src_data = [
...
def test_relative_jump_out_of_range(assembler):
    """相対ジャンプ（範囲外）のエラーテスト"""
...
def test_conditional_jump(assembler):
    """条件付きジャンプ命令のアセンブルテスト"""
    src_data = [
        {"line": 1, "asm": ["org", "0x100"]},
        {"line": 2, "asm": ["LABEL", ":"]},
...
def test_expression_with_subtraction(assembler):
    """減算を含む数式評価のテスト"""
    src_data = [
        {"line": 1, "asm": ["org", "0x100"]},
        {"line": 2, "asm": ["L1", ":"]},
...
def test_expression_with_parentheses(assembler):
    """括弧を含む数式評価のテスト"""
    src_data = [
        {"line": 1, "asm": ["VAL", ":", "equ", "10"]},
